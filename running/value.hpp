...
namespace OPL_VALUE {
	...
	struct OPL_BasicValue {
		...
		virtual void __elementset__(OPL_BasicValue *, OPL_BasicValue *) {
			operator_not_impl_error("", "__elementset__");
		}
		
		virtual OPL_BasicValue *__elementget__(OPL_BasicValue *) {
			operator_not_impl_error("", "__elementget__");
			return nullptr;
		}
		
		virtual OPL_BasicValue *__memberget__(int) {
			operator_not_impl_error("", "__memeberget__");
			return nullptr;
		}
		
		virtual void __memberset__(int, OPL_BasicValue *) { operator_not_impl_error("", "__memberset__"); }
		
		virtual OPL_BasicValue *__copy__() {
			operator_not_impl_error("", "__copy__");
			return nullptr;
		}
	...
	inline int get_int(STACK_VALUE *arg) {
		if (arg->is_heap_ref) {
	...
	struct Frame {
		...
		inline int *get_start() {
			if (codes->op_codes.empty() || is_build_in) {
				std::cout << "In SubProc <Frame.get_start>, want get start, but "
				          << ((is_build_in) ? "the function is a build-in function" : "code is empty") << std::endl;
				exit(-1);
			}
	...
	inline void disassemble_instruction(Chunk *chunk, int *offset) {
	...
	inline void disassemble_chunk(Chunk *chunk, const char *name) {
		printf("== %s ==\n", name);
		int offset = 0;
